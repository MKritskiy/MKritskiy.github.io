function getPrecipations(data, length){
    ...
    for(let i=0; i<length;i++){
      if (i<7){
        ...
        avgPrecipationAmplitude = precipations.reduce((sum, current, index) => (sum*(index+1) + current)/(index+2), 0);
        precipations.push(Math.floor((precipations[i-1] + Math.random()*avgPrecipationAmplitude*2-avgPrecipationAmplitude)*100)/100);
      }  
    }
    return precipations;
}
...
function renderPrecipationsCanvas(data) {
      ...
      datasets: [
        {
          ...
          data: precipations.map((item,index)=>{
            return {y: item, x:new Date(90000000*index).toISOString().slice(0,11) + '00:00'};
          }),
  ...
  window.chart = new Chart(document.getElementById("canvas_precipation"), config,)
}
