...
function getAverageAmplitude(data){
    ...
    let sum=0;
    for(let i = 0; i< data.length-1; i++){
        sum += Math.abs(parseInt(data[i])-parseInt(data[i+1]));
    }
    avgAmplitude = sum/data.length;
    return Math.floor(avgAmplitude);
}
...
{
    let forecastDataContainer = document.querySelector('.forecast__month');
    let forecasts = '';
    ...
    for(let i =0; i<data.daily.time.length; i++)
    {
        ...
        else if (tmp == "51" || tmp=="53" || tmp=="55" || tmp=="56" || tmp=="57") icon="10d";
        else if (tmp == "61" || tmp=="63" || tmp=="65" || tmp=="66" || tmp=="67") icon="09d";
        else if (tmp == "71" || tmp=="73" || tmp=="75" || tmp=="77") icon="13d";

        let max_temp = i==0 ? "Макс " + getTemperature(data.daily.temperature_2m_max[i]) : getTemperature(data.daily.temperature_2m_max[i]);
        let min_temp = i==0 ? "Мин " + getTemperature(data.daily.temperature_2m_min[i]) : getTemperature(data.daily.temperature_2m_min[i]);
...
function renderRandForecast(data){
    let forecastDataContainer = document.querySelector('.forecast__month');
    let accurateMaxTemp = [...document.querySelectorAll('.forecast__max__temperature')];
    let avarageMaxTemp = accurateMaxTemp.reduce((sum, current, index) => (index==0 ? (sum*(index+1) + parseInt(current.innerHTML.slice(4,-1)))/(index+2) : (sum*(index+1) + parseInt(current.innerHTML.slice(0,-1)))/(index+2)), 0);
    let averageMaxTempAmplitude = getAverageHtmlAmplitude(accurateMaxTemp);
    let accurateMinTemp = [...document.querySelectorAll('.forecast__min__temperature')];
    let avarageMinTemp = accurateMinTemp.reduce((sum, current, index) => (index==0 ? (sum*(index+1) + parseInt(current.innerHTML.slice(4,-1)))/(index+2) : (sum*(index+1) + parseInt(current.innerHTML.slice(0,-1)))/(index+2)), 0);
    let averageMinTempAmplitude = getAverageHtmlAmplitude(accurateMinTemp);
    let forecasts = forecastDataContainer.innerHTML;
    let avgWeathercode = data.daily.weathercode.reduce((sum, current, index) => (sum*(index+1) + parseInt(current))/(index+2), 0)
    ...
    for(let i = 0; i<24; i++)
    {
        tmp = (Math.floor(avgWeathercode) +  Math.floor(Math.random()*avgWeathercodeAmplitude)).toFixed();
        //Иконки
        let icon;
        if (tmp == "0") icon="01d"; 
        else if (tmp == "1" || tmp=="2") icon="02d";
        else if (tmp == "3" || parseInt(tmp)<parseInt('45')) icon="03d";
        else if (tmp == "45" || parseInt(tmp)>parseInt('45') && parseInt(tmp)<parseInt('48')|| tmp == "48" || parseInt(tmp)<parseInt('51')) icon="50d";
        else if (tmp == "51" || parseInt(tmp)>parseInt('51') && parseInt(tmp)<parseInt('53') || tmp=="53" || parseInt(tmp)>parseInt('53') && parseInt(tmp)<parseInt('55')|| tmp=="55" || tmp=="56" || tmp=="57" || parseInt(tmp)<parseInt('61')) icon="10d";
        else if (tmp == "61" || parseInt(tmp)>parseInt('61') && parseInt(tmp)<parseInt('63') || tmp=="63" || parseInt(tmp)>parseInt('63') && parseInt(tmp)<parseInt('65') ||tmp=="65" || tmp=="66" || tmp=="67" || parseInt(tmp)<parseInt('71')) icon="09d";
        else if (tmp == "71" || parseInt(tmp)>parseInt('71') && parseInt(tmp)<parseInt('73') || tmp=="73" || parseInt(tmp)>parseInt('73') && parseInt(tmp)<parseInt('73') || tmp=="75" || tmp=='76'|| tmp=="77") icon="13d";
        //Макс мин температура
        let max_temp = Math.floor(avarageMaxTemp) +  Math.floor(Math.random()*averageMaxTempAmplitude*2 - averageMaxTempAmplitude);//getTemperature(data.daily.temperature_2m_max[i]);
        let min_temp = Math.floor(avarageMinTemp) +  Math.floor(Math.random()*averageMinTempAmplitude*2 - averageMinTempAmplitude)//getTemperature(data.daily.temperature_2m_min[i]);
